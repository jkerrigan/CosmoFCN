...
class FCN21CM():
    ...
    def FCN(self):
        ...
        self.s2 = Dropout(rate=0.3)(stacked_layer(self.s1,ksize=3,fsize=128,psize=2)) # 16,16,10,128
        self.s3 = Dropout(rate=0.3)(stacked_layer(self.s2,ksize=3,fsize=256,psize=2)) # 4,4,5,256 #dropout added here 
        self.fc1 = Dropout(rate=0.3)(stacked_layer(self.s3,ksize=3,fsize=512,psize=2)) # 1,1,1,2048
        self.out = Dropout(rate=0.3)(Conv2D(filters=3,kernel_size=3,padding='same')(self.fc1))
    ...
    def probe_FCN(self,layer2output=None,weights=None):
        ...
        self.s1_ = stacked_layer(inputs,ksize=5,fsize=16,psize=4,weights=weights[:6])
        if layer2output == '1':
            model = Model(inputs=inputs,outputs=self.s1_)
            return model
#        print('w2',np.shape(weights[1]))
        self.s2_ = stacked_layer(self.s1_,ksize=5,fsize=32,psize=2,weights=weights[6:12])
        if layer2output == '2':
            model = Model(inputs=inputs,outputs=self.s2_)
            return model
#        print('w3',np.shape(weights[2]))
        self.s3_ = stacked_layer(self.s2_,ksize=3,fsize=64,psize=2,weights=weights[12:18])
        if layer2output == '3':
            model = Model(inputs=inputs,outputs=self.s3_)
            return model
        self.fc1_ = stacked_layer(self.s3_,ksize=3,fsize=128,psize=2,weights=weights[18:24])
        if layer2output == '4':
